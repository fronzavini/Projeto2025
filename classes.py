...
class Venda:
    ...
    @staticmethod
    def criarVenda(cliente, funcionario, produtos, valorTotal, dataVenda, entrega, dataEntrega, id_pagamento_mp=None):
        conexao = conectar_banco()
        cursor = conexao.cursor()
        query = '''
            INSERT INTO vendas (cliente, funcionario, produtos, valorTotal, dataVenda, entrega, dataEntrega, status, id_pagamento_mp)
            VALUES (%s, %s, %s, %s, %s, %s, %s, 'pendente', %s)
        '''
        cursor.execute(query, (cliente, funcionario, str(produtos), valorTotal, dataVenda, entrega, dataEntrega, id_pagamento_mp))
        conexao.commit()
        cursor.close()
        conexao.close()
