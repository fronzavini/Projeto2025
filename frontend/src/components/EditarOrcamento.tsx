...
type ItemVendido = {
  nome: string;
  quantidade: number;
  valorUnitario: string; // string no dado original
  valorTotal: string;
};
...
export default function EditarOrcamento({
  ...
  const [cliente, setCliente] = useState<string>(orcamento.nomeCliente || "");
  const [clienteBusca, setClienteBusca] = useState("");

  // Tipo entrega pode ser "Entrega" ou "Retirada" (capitalizado)
  ...
  const [frete, setFrete] = useState<number>(0); // Sem frete no dado original, ajustar se quiser
  ...
  const handleQuantidade = (index: number, delta: number) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Monta objeto atualizado no formato do Pedido
    const orcamentoAtualizado: Pedido = {
      ...orcamento,
      nomeCliente: cliente,
      ...
      // Você pode incluir valorTotal, subtotal e desconto recalculados
