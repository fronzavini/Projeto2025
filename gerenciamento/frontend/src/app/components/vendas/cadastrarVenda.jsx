...
export default function CadastrarVenda({ onClose }) {
  ...
  const carregarClientes = async () => {
    try {
      const res = await fetch("http://192.168.18.155:5000/listar_clientes");
      const data = await res.json();
  ...
  const carregarFuncionarios = async () => {
    try {
      const res = await fetch("http://192.168.18.155:5000/listar_funcionarios");
      const data = await res.json();
  ...
  async function criarPagamentoPix({ total, pedidoId, email }) {
    const resp = await fetch("http://192.168.18.155:5000/create_pix_payment", {
      method: "POST",
  ...
  const handleConfirmarVenda = async () => {
    ...
    try {
      ...
      const res = await fetch("http://192.168.18.155:5000/criar_venda", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      for (const item of produtos) {
        ...
        if (prod) {
          ...
          await fetch(`http://192.168.18.155:5000/editar_produto/${item.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
