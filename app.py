...
import mercadopago
from config_mp import MERCADO_PAGO_ACCESS_TOKEN
...
def gerar_pagamento():
    ...
    pagamento = {
        "items": [
            {
                "title": dados.get("descricao", "Compra BellaDonna"),
                "quantity": 1,
    ...
    resultado = sdk.preference().create(pagamento)
...
def checkout():
    ...
    # 2. Criar pagamento Mercado Pago
    pagamento = {
        "items": [
            {
                "title": "Compra Floricultura BellaDonna",
                "quantity": 1,
                "unit_price": float(dados["valorTotal"])
            }
        ],
        "external_reference": str(id_venda),  # ESSENCIAL para identificar a venda depois!
        "auto_return": "approved",
        "back_urls": {
            "success": "http://localhost:3000/vendas/sucesso", 
            "failure": "http://localhost:3000/vendas/falha",
            "pending": "http://localhost:3000/vendas/pendente"
        }
    }
...
""""
Isso é pro webhook do mercado livre, ai ele notifica externamente também. Não consegui fazer funcionar ainda
@app.route('/webhook_mp', methods=['POST'])
def webhook_mp():
    noti = request.json
    print("Webhook recebido:", noti)

    if noti and 'data' in noti and 'id' in noti['data']:
        pagamento_id = noti["data"]["id"]
        pagamento_info = sdk.payment().get(pagamento_id)
        status = pagamento_info["response"]["status"]
        venda_id = pagamento_info["response"]["external_reference"]

        # Atualiza status no banco
        conexao = conectar_banco()
        cursor = conexao.cursor()
        query = "UPDATE vendas SET status = %s, id_pagamento_mp = %s WHERE id = %s"
        cursor.execute(query, (status, pagamento_id, venda_id))
        conexao.commit()
        cursor.close()
        conexao.close()
